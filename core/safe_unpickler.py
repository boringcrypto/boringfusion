...
def torch_load(
    ...
    **pickle_load_args
    ...
    with _open_file_like(f, 'rb') as opened_file:
        if torch.serialization._is_zipfile(opened_file):
            # The zipfile reader is going to advance the current file position.
            # If we want to actually tail call to torch.jit.load, we need to
            # reset back to the original position.
            orig_position = opened_file.tell()
            with torch.serialization._open_zipfile_reader(opened_file) as opened_zipfile:
                if torch.serialization._is_torchscript_zip(opened_zipfile):
                    opened_file.seek(orig_position)
                    return torch.jit.load(opened_file)
                return torch.serialization._load(opened_zipfile, map_location, pickle_module, **pickle_load_args)
        return torch.serialization._legacy_load(opened_file, "cpu", pickle_module, **pickle_load_args)
...
def _get_allowed_globals():
    ...
    rc: Dict[str, Any] = {
        ...
        "pytorch_lightning.callbacks.model_checkpoint.ModelCheckpoint": DummyClass
    }
    # dtype
    for t in [
        # torch.complex32,
    ...
    def load(self):
        ...
        while True:
            ...
            if key[0] == GLOBAL[0]:
                ...
                elif str(type(inst)) == "<class 'numpy.dtype[float64]'>":
                    inst.__setstate__(state)
